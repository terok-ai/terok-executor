...
class Preflight:
    ...
    def check_sandbox_services(self) -> CheckResult:  # noqa: PLR6301
        ...
        host service any more.  The per-container supervisor (spawned by
        the terok-sandbox OCI hook) embeds the vault proxy *and* serves
        the git gate in-process, both starting on demand.  The only
        host-side service that must exist before a launch is therefore
        the shield OCI hooks; the git binary that drives gate mirrors is
        surfaced separately by [`check_git`][terok_executor.preflight.Preflight.check_git].
        """
