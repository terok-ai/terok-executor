...
def _inject_vault_tokens(
    ...
    *,
    ...
    ``VaultManager`` lifecycle to probe.  The per-container
    supervisor (spawned by the terok-sandbox OCI hook on container
    start) embeds the vault proxy and binds to the socket/port
    written into the sidecar config.  Token creation only needs DB
    access; the broker port is a config-derived constant.
    *vault_required* is retained for callers that want DB-level
    hard failure (e.g. terok project mode) but no longer maps to a
