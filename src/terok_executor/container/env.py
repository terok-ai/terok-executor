...
def _inject_vault_tokens(
    ...
    *credential_set* selects which vault DB namespace to query — the auth
    ...
    Post-supervisor-refactor: there is no global vault daemon, no
    ``VaultManager`` lifecycle to probe.  The per-container supervisor
    (spawned by the terok-sandbox OCI hook on container start) embeds
    the vault proxy and binds to the socket/port written into the sidecar
    config.  Token creation only needs DB access; the broker port is a
    config-derived constant.  *vault_required* therefore gates DB-level
    hard failure (terok project mode) versus soft-fail to an empty dict
    (standalone default) — it no longer maps to a daemon-liveness probe.
    """
    ...
    try:
        ...
        # path); legacy fallback to the host-singleton ``cfg`` field
        # for tests + ad-hoc callers that haven't migrated yet.  In
        # production the runner always provides ``per_container`` so
        # concurrent containers get distinct ports.
        port = (
    ...
    if ssh_token:
        ...
        if use_socket:
            ...
            # Per-container port (production); legacy fallback to cfg
            # (tests).  See ``port`` resolution above for the same shape.
            ssh_port = (
