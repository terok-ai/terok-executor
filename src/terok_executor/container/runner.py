...
class AgentRunner:
    ...
    def launch_prepared(
        ...
        *,
        ...
        # on the singletons baked into ``cfg``.
        ...
        # host-singleton ``cfg.token_broker_port`` — that field is
        # legacy and ignored by the launch flow now.
    ...
    def _setup_gate(self, repo_url: str) -> tuple[str, str]:
        ...
        The gate itself now runs inside the per-container supervisor, so
        there is no host daemon to start — the token travels to the
        container via the sidecar (the caller surfaces it as
        ``TEROK_GATE_TOKEN`` so the launch path wires it into the
        sidecar).  The container clones from the returned URL; shield
