...
``uninstall`` / ``status`` lifecycle to manage.  The supervisor spawns
on container start via the terok-sandbox OCI hook and reads the per-
container sidecar to bind its proxy; sandbox keeps only ``unlock`` /
``lock`` / ``passphrase`` (passphrase-tier CRUD on the DB).
...
Both verbs operate on host-side files only (no daemon round-trip).
"""
...
def _build_sandbox_tree() -> CommandTree:
    ...
    No overlays: every sandbox verb flows through unchanged.  After
    the per-container-supervisor refactor there is no host-side
    daemon lifecycle to wrap (no ``start`` / ``stop`` / ``install`` /
    ``uninstall`` / ``status`` to override), so executor's vault
    surface is now pure extension.
    """
