...
def apply_shared_config_patches(
    ...
    *,
    ...
    # See note in ``apply_shared_config_patch_for_provider`` — shared
    # mounts force a singleton port here regardless of per-container
    # allocation.
    location = resolve_vault_location(SandboxConfig().token_broker_port)
...
def resolve_vault_location(token_broker_port: int | None = None) -> VaultLocation:
    ...
    URL is always ``http://localhost:9419/v1`` — the in-container
    loopback bridge runs in both transports and forwards to the
