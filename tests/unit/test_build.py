...
class TestTemplateRendering:
    ...
    def test_l0_hardens_sshd_to_pubkey_only_dev_only_no_forwarding(self) -> None:
        ...
        non-root account, pubkey-only, no forwarding of any kind."""
        content = render_l0("ubuntu:24.04", family="deb")
        for directive in (
            "PermitRootLogin no",
    ...
    def test_l0_ships_empty_authorized_keys_placeholder(self) -> None:
        ...
        assert "KRUN_HOST_PUBKEY" not in content

    def test_l0_carries_no_per_installation_secrets(self) -> None:
        """The rendered L0 is byte-identical regardless of which host
        rendered it — no embedded hostnames, fingerprints, or keys.
        Pins the "L0 image caches identically across hosts" property."""
        a = render_l0("ubuntu:24.04", family="deb")
        b = render_l0("ubuntu:24.04", family="deb")
        assert a == b
